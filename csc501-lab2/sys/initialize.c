...
// define a variable to control debugging
int debug = 0;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	// initialize backing store map table.
	init_bsm();
	// initialize frame map table
	init_frm();
	// install page fault handler
	set_evec(14, pfintr);
	// initialize gloabl 4 page tables
	init_gpt();
	// new pd for null process
	create_pd(NULLPROC);
	// change pdbr

...
void test3 ()
{
    ...
    //kprintf("-allocate all the locks(NLOCKS)\n");
        for (index = 0; index < NLOCKS; index++) {
            ...
            assert (lck[index] != SYSERR,"Test 3 FAILED1\n");
        }
        ...
        assert (last_lck == SYSERR,"Test 3 FAILED2\n");
...
void test4 ()
{
    ...
    lock will share the same location as the just deleted one*/
        ...
        lck[0]   = lcreate ();
        assert (lck[0] != SYSERR,"Test 4 FAILED1\n");
        ...
        assert (ret == SYSERR,"Test 4 FAILED2\n");
...
void test6 ()
{
        ...
        assert (ret == OK,"Test 6 FAILED1\n");

        ret = releaseall (3, lck[1], lck[3], lck[2]);
        assert (ret == OK,"Test 6 FAILED2\n");
...
void test11 ()
{
        ...
        lck1  = lcreate ();
        assert (lck1 != SYSERR,"Test 11 FAILED11\n");
    
        lck2  = lcreate ();
        assert (lck2 != SYSERR,"Test 11 FAILED22\n");
        ...
        assert (getprio (wr2) == 25,"Test 11 FAILED0\n");
    
        //kprintf("-start reader C, reader C<30> will block on lock1\n");
        resume (rd1);
        sleep (1);
        assert (getprio (wr2) == 30,"Test 11 FAILED1\n");
        assert (getprio (wr1) == 30,"Test 11 FAILED2\n");
